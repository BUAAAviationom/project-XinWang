...
class BaseAgent:
    def __init__(self, log_dir, num_steps=100000, batch_size=128, lr=0.00001, memory_size=20000,
                 ...
                 eval_interval=1000, cuda=True, seed=0, mode='sac', num_packet=600, uav_speed=20):
        ...
        self.alpha_ori = str(alpha)
        self.speed = str(uav_speed)
    ...
    def run(self):
        while self.steps <= self.num_steps:
            ...
            #     self.file_write('SAC_' + self.alpha_ori + 'alpha_seed'+self.seed+'_return.txt', episode_return)
            #     self.file_write('SAC_' + self.uav_speed + 'speed_seed' + self.seed + '_time.txt', self.env.delay)
        'end train'
        # self.packet_list()
        self.draw_uav_way()
    ...
    def packet_list(self):
        ...
        self.file_write('SAC_packet_UAV_seed' + self.seed + '_packet.txt', np.sum(self.test_env.queue_len))
        done = False
        while not done:
            ...
            state = next_state
            self.file_write('SAC_packet_noUAV_seed' + self.seed + '_packet.txt', np.sum(self.test_env.queue_len))

    def draw_uav_way(self):
        self.test_env.reset()
        state = self.test_env.get_state()
        action_list = []  # just print
        done = False
        'draw before run'
        for x in range(self.test_env.num_node):  # 画出routing路线
            plt.plot([self.test_env.node_loc[x][0], self.test_env.node_loc[self.test_env.routing[x]][0]],
                     [self.test_env.node_loc[x][1], self.test_env.node_loc[self.test_env.routing[x]][1]], c='b')
        plt.scatter(self.test_env.node_loc.T[0], self.test_env.node_loc.T[1], c='k')  # 画node位置
        plt.scatter(self.test_env.node_loc[-1][0], self.test_env.node_loc[-1][1], c='r')  # 画AP位置，红点标记
        plt.scatter(self.test_env.init_loc_x, self.test_env.init_loc_y, s=50, c='r', marker='P')  # 画UAV初始位置
        ...
        while not done:
            ...
            node_access = action // 4  # 节点接入为0~num_node-1
            plt.scatter(self.test_env.tra[-1, 0], self.test_env.tra[-1, 1], s=50, c='peru', marker='P')   # UAV新位置
            plt.plot([self.test_env.tra[-2, 0], self.test_env.tra[-1, 0]],
                     [self.test_env.tra[-2, 1], self.test_env.tra[-1, 1]], c='r')                       # UAV trajectory
            plt.plot([self.test_env.tra[-1, 0], self.test_env.node_loc[node_access, 0]],
                     [self.test_env.tra[-1, 1], self.test_env.node_loc[node_access, 1]],
                     c='c', linestyle='--', linewidth=0.7)                                              # node routing
        plt.xlim(0, 200)
        plt.ylim(0, 200)
    ...
    def calc_entropy_loss(self, entropies, weights):
        ...
        entropy_loss = -torch.mean(self.log_alpha.exp() * (self.target_entropy - entropies) * weights)
        return entropy_loss
    ...
    def evaluate(self):
        ...
        'file write'
        # self.file_write('SAC_' + self.alpha_ori + 'alpha_' + self.num_packet + 'packet_seed' + self.seed + '_alpha.txt', self.alpha.item())
        # self.file_write('SAC_' + self.train_steps + 'steps_seed' + self.seed + '_entropy.txt', ave_entropy)
        # self.file_write('SAC_near_' + self.speed + 'speed_seed' + self.seed + '_evatime.txt', ave_delay)
        # self.file_write('SAC_' + self.num_packet + 'packet_seed' + self.seed + '_evatime.txt', ave_delay)
...
def main(alpha, seed, num_packet, num_steps, speed):
    ...
    args = {'num_steps': num_steps, 'batch_size': batch_size, 'lr': lr, 'memory_size': start_steps, 'gamma': gamma,
            ...
            'num_packet': num_packet, 'uav_speed': speed}
