...
class Env:
    ...
    def get_access_node(self):
        weighted_dis = []
        for i in range(self.num_node):
            weighted_dis += [np.linalg.norm(np.append(self.node_loc[i] - self.tra[-1], self.params_env['H']))]  # 距离
        access_node = weighted_dis.index(min(weighted_dis))
        return access_node
    ...
    def step(self, action):
        ...
        node_access = self.get_access_node()   # 节点接入为0~num_node-1
